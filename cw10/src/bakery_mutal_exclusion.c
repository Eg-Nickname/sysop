// ======================================================================================
// Program ralizuje probelm wzajemnego wykluczania wątków, przy uzyciu algorytmu
// piekarni. Przyjmuje jao argumenty liczbę wątków oraz liczbe ich sekcji
// krytycznych.
// ======================================================================================
...
int main(int argc, char **argv) {
    ...
    // Alocate bufffers for bakery algorithm
    choosing = malloc(sizeof(char) * THREAD_COUNT);
    threads_numbers = malloc(sizeof(int) * THREAD_COUNT);
...
void *thread_fn(void *arg) {
    ...
    for (int i = 0; i < CRIT_SECTION_COUNT; i++) {
        ...
        for (int k = 0; k < THREAD_COUNT; k++) {
            if (max_num < threads_numbers[k]) {
                max_num = threads_numbers[k];
            }
        }
        threads_numbers[thread_num] = max_num + 1;
        ...
        for (int j = 0; j < THREAD_COUNT; j++) {
            ...
            // czekaj
            while ((threads_numbers[j] != 0) &&
                   ((threads_numbers[j] < threads_numbers[thread_num]) ||
                    (threads_numbers[j] == threads_numbers[thread_num] &&
                     j < thread_num))) {
            } // czekaj
        }
        ...
        local_counter++;
        // Random sleep before writing back to global counter and exiting
        // crit section
        rsleep();
